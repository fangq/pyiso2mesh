...
__all__ = ["affinemap", "meshinterp", "meshremap", "proj2mesh", "dist2surf", "regpt2surf"]
...
def meshinterp(fromval, elemid, elembary, fromelem, initval=None):
    ...
    allval = np.reshape(fromval[fromelem[elemid[idx], :], :], (len(idx), elembary.shape[1], fromval.shape[1]))

    # Perform linear interpolation using barycentric coordinates
    tmp = np.array([np.sum(elembary[idx, :] * x, axis=1) for x in np.rollaxis(allval, 2)])
...
def meshremap(fromval, elemid, elembary, toelem, nodeto):
    ...
    nodeval = np.repeat(fromval[:, np.newaxis, :], elembary.shape[1], axis=1) * \
              np.repeat(elembary[:, np.newaxis, :], fromval.shape[1], axis=1).transpose(0, 2, 1)
...
def point_to_triangle_distance(point, tri):
    ...
    distance = np.linalg.norm(point - nearest_point) if nearest_point is not None else np.inf
