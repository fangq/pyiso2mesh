...
def v2s(img, isovalues, opt=None, method=None):
    ...
    no, el, regions, holes = vol2surf(
        img, np.arange(img.shape[0]), np.arange(img.shape[1]), np.arange(img.shape[2]), opt, 1, method, isovalues
    )
    
    return no, el, regions, holes

def s2m(v, f, keepratio=None, maxvol=None, method='tetgen', regions=None, holes=None, *args):
    """
    ...
    if args:
        node, elem, face = surf2mesh(v, f, [], [], keepratio, maxvol, regions, holes, 0, method, *args)
    else:
...
def vol2mesh(img, ix, iy, iz, opt, maxvol, dofix, method='cgalsurf', isovalues=None):
    ...
    if method == 'cgalmesh':
        ...
        if len(np.unique(vol)) > 64 and dofix == 1:
            raise ValueError("CGAL mesher does not support grayscale images. Use 'cgalsurf' for grayscale volumes.")
...
def vol2surf(img, ix, iy, iz, opt, dofix, method='cgalsurf', isovalues=None):
    ...
    if img is not None:
        ...
        for i in range(maxlevel):
            levelmask = (newimg >= isovalues[i]) if i == maxlevel - 1 else (newimg >= isovalues[i]) & (newimg < isovalues[i + 1])
            levelno, levelel = binsurface(levelmask.astype(np.int8))
            
            if levelel.size > 0:
                seeds = surfinterior(levelno, levelel) if not opt.get('autoregion', False) else surfinterior(levelno, levelel)
                if len(seeds) > 0:
        ...
        for i in range(maxlevel):
            if method == 'simplify':
                ...
                perturb = -perturb if np.all(newimg > isovalues[i] - perturb) else perturb
                
                v0, f0 = vol2restrictedtri(newimg, isovalues[i] - perturb, regions[i, :], np.sum(newdim ** 2) * 2, 30, radbound, distbound, maxsurfnode)

            if el == []:
                el = np.hstack([f0, (i + 1) * np.ones((f0.shape[0], 1), dtype=int)])
                no = v0
            else:
                el = np.vstack([el, np.hstack([f0 + len(no), (i + 1) * np.ones((f0.shape[0], 1), dtype=int)])])
                no = np.vstack([no, v0])
...
def surf2mesh(v, f, p0, p1, keepratio, maxvol, regions=None, holes=None, forcebox=0, method='tetgen', cmdopt=''):
    print('Generating tetrahedral mesh from closed surfaces...')
    
    if keepratio > 1 or keepratio < 0:
        print('Warning: keepratio must be between 0 and 1. No simplification will be performed.')
...
def surf2vol(node, face, xi, yi, zi, **kwargs):
    ...
    for i in range(len(elabel)):
        ...
        im = np.logical_or(im, np.rollaxis(surf2volz(node[:, [2, 0, 1]], fc[:, :3], zi, xi, yi), 1))
        im = np.logical_or(im, np.rollaxis(surf2volz(node[:, [1, 2, 0]], fc[:, :3], yi, zi, xi), 2))
...
def cgalv2m(vol, opt=None, maxvol=None):
    ...
    if not (vol.dtype == bool or vol.dtype == np.uint8):
        raise ValueError('cgalmesher can only handle uint8 volumes. Convert your image to uint8 first.')
    ...
    if not os.path.exists(mwpath('post_cgalmesh.mesh')):
        raise FileNotFoundError(f'Output file was not found. Failure encountered when running command: {cmd}')
    ...
    if len(node) > 0:
        node, elem, face = sortmesh(node[0, :], node, elem, np.arange(4), face, np.arange(3))
...
def surf2volz(node, face, xi, yi, zi):
    ...
    for i in iz:
        ...
        for j in range(enum):
            ...
            posx = np.floor((e0[0] + np.arange(length + 1) * dd[0] - xi[0]) / dx0).astype(int)
            posy = np.floor((e0[1] + np.arange(length + 1) * dd[1] - yi[0]) / dy0).astype(int)
            pos = np.column_stack((posx, posy))
            valid_idx = np.all((posx > 0, posy > 0, posx < len(xi), posy < len(yi)), axis=0)
            pos = pos[valid_idx]
...
def removedupelem(elem):
    ...
    _, unique_indices, counts = np.unique(sorted_elem, axis=0, return_index=True, return_counts=True)
...
def removeisolatedsurf(v, f, maxdiameter):
    ...
    if fnew.shape[0] != f.shape[0]:
        print(f"Removed {f.shape[0] - fnew.shape[0]} elements of small isolated surfaces")
    ...
    for i in range(0, len(args), 2):
        ...
        if el.shape[1] == 5 or el.shape[1] == 4:
            ...
            if el.shape[1] == 4 and X >= 0:
                el = np.column_stack((el, np.ones((el.shape[0], 1), dtype=int) * (i // 2 + 1)))
...
def meshrefine(node, elem, *args):
    ...
    newnode, newelem, newface = node, elem, face  # Placeholder, actual implementation needs external tools
...
def surfboolean(node, elem, *args):
    ...
    for i in range(0, len_args, 3):
        ...
        if status != 0:
            raise RuntimeError(f'Surface boolean operation failed: {cmd}\nError: {outstr}')
...
def mcpath(fname, ext=None):
    ...
    if platform.system() == "Windows" and '64' in platform.architecture()[0] and not fname.endswith('_x86-64'):
...
def mwpath(fname=""):
    ...
    if not p:
        tdir = os.path.abspath(os.path.join(os.sep, 'tmp'))  # Use default temp directory
        if username:
