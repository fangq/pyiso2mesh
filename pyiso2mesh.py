...
#          with dimensions of (size(v,1),3)
    ...
    if len(idx) < nn:
        print('Warning: found interior nodes, their norms will be set to zeros; to remove them, please use removeisolatednodes.m from iso2mesh toolbox')
...
def plotsurf(node, face, *args):
    ...
    if len(sys.argv) >= 2:
        ...
        if isinstance(face, list):
            sc = csr_matrix((10, 3))  # face colormap
            sc[0:10, :] = np.random.rand(10, 3)
            length = len(face)
            ...
            for i in range(newlen):
                ...
                try:
                    ...
                    if subface.shape[0] > 1 and subface.ndim == 2:
                        subface = subface.T
                    h.append(plt.Patch(vertices=node, faces=subface, facecolor=sc[i, :], *args))
                except:
                    for j in range(len(newsurf[i])):
                        h.append(plt.Patch(vertices=node, faces=newsurf[i][j], facecolor=sc[i, :], *args))
        else:
            if face.shape[1] == 4:
                tag = face[:, 3]
                types = np.unique(tag)
                ...
                for i in range(len(types)):
                    if node.shape[1] == 3:
                        h.append(plotasurf(node, face[tag == types[i], 0:3], facecolor=np.random.rand(3, 1), *args))
                    else:
...
def plotasurf(node, face, *args):
    if face.shape[1] <= 2:
        ...
        if node.shape[1] == 4:
          h = plt.trisurf(face[:, 0:3], node[:, 0], node[:, 1], node[:, 2], node[:, 3], *args)
        else:
          ...
          # Creating plot
          trisurf = h.plot_trisurf(node[:,0], node[:,1], face, node[:,2],cmap=my_cmap)
...
def varargin2struct(*args):
    ...
    while i < length:
        if isinstance(args[i], dict):
            ...
            raise ValueError("input must be in the form of ...,'name',value,... pairs or structs")
        i += 1
